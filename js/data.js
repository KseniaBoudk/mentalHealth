...
/* =====================================================================
   1c. REAL DATA — specialist psychiatric care, from js/real_mh_data.js's
...
const REAL_PSYCH = (() => {
  ...
  const idxAll = {};  // idxAll[county][sex][year] = {value,count}  -- the register's own "0-85+"
  for (const row of rows) {
...
/* =====================================================================
   ...
   js/real_mh_data.js's REAL_HLV_MH (fetch_folkhalsodata_hlv.py).
...
function realCellPsych(regionCode, year, ageIdx, sex) {
  if (!REAL_PSYCH.active) return undefined;
  const county = regionCode === "SE" ? "00" : regionCode;
  const row = REAL_PSYCH.idx[county] && REAL_PSYCH.idx[county][ageIdx] &&
              REAL_PSYCH.idx[county][ageIdx][sex] && REAL_PSYCH.idx[county][ageIdx][sex][year];
  return row ? realRowToCell(row) : null;
}
function realTotalPsych(regionCode, year, sex) {
  if (!REAL_PSYCH.active) return undefined;
  const county = regionCode === "SE" ? "00" : regionCode;
  const row = REAL_PSYCH.idxAll[county] && REAL_PSYCH.idxAll[county][sex] && REAL_PSYCH.idxAll[county][sex][year];
  return row ? realRowToCell(row) : null;
}
